
public class StringOptims {
	
	private static final int LOOP_COUNT = 10_000_000;
	
	public static void withoutOptims() {
		long begin = System.currentTimeMillis();
		String part1 = "part1";
		double part2 = 3.141592654;
		int part3 = 123;
		char part4 = '\u38FA';
		
		for (int i = 0; i < LOOP_COUNT; i++) {
			String fullMessage = "Begin - ";
			fullMessage += part1;
			fullMessage += " - ";
			fullMessage += part2;
			fullMessage += " - ";
			fullMessage += part3;
			fullMessage += " - ";
			fullMessage += part4;
			fullMessage += " - End";
			//System.out.println(fullMessage);
		}
		long end = System.currentTimeMillis();
		System.out.println("Duration : " + (end-begin) + "ms");
	}
	
	public static void optimsThreadSafe() {
		long begin = System.currentTimeMillis();
		String part1 = "part1";
		double part2 = 3.141592654;
		int part3 = 123;
		char part4 = '\u38FA';
		
		for (int i = 0; i < LOOP_COUNT; i++) {
			StringBuffer buffer = new StringBuffer("Begin - ");
			buffer.append(part1);
			buffer.append(" - ");
			buffer.append(part2);
			buffer.append(" - ");
			buffer.append(part3);
			buffer.append(" - ");
			buffer.append(part4);
			buffer.append(" - End");
			String fullMessage = buffer.toString();
			//System.out.println(fullMessage);
		}
		long end = System.currentTimeMillis();
		System.out.println("Duration : " + (end-begin) + "ms");
	}
	
	public static void optimsThreadUnsafe() {
		long begin = System.currentTimeMillis();
		String part1 = "part1";
		double part2 = 3.141592654;
		int part3 = 123;
		char part4 = '\u38FA';
		
		for (int i = 0; i < LOOP_COUNT; i++) {
			StringBuilder builder = new StringBuilder("Begin - ");
			builder.append(part1);
			builder.append(" - ");
			builder.append(part2);
			builder.append(" - ");
			builder.append(part3);
			builder.append(" - ");
			builder.append(part4);
			builder.append(" - End");
			String fullMessage = builder.toString();
			//System.out.println(fullMessage);
		}
		long end = System.currentTimeMillis();
		System.out.println("Duration : " + (end-begin) + "ms");
	}
	
	public static void main(String[] args) {
		withoutOptims();
		optimsThreadSafe();
		optimsThreadUnsafe();
		//Attention, les concaténations en lignes sont déjà optimisés en Builder.
		//Dans le cadre d'une concaténation dans une boucle : 
		//Utiliser un builder ou un buffer.
	}

}
